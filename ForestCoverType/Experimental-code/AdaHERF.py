...
from elm import SimpleELMClassifier,ELMClassifier
...
class AdaHERF(object):
    ...
    AdaHERF
    """
    #Orig - n_classifiers=35
    def __init__(self, n_classifiers=23):
...
#         V3  - modded to remove m=m()
        self._listclassifiers = [SimpleELMClassifier(),
		GaussianNB(),
	        KNeighborsClassifier(n_neighbors=3, p=2, weights='distance'),
	        SGDClassifier(n_iter= 17, loss='hinge', l1_ratio= 0.6, penalty= 'l1', learning_rate= 'optimal'),
	        RandomForestClassifier(min_samples_leaf= 2, max_depth= 13, criterion=
	        'entropy', min_samples_split= 1, n_estimators= 300),
	        GradientBoostingClassifier(max_depth= 11, max_features= 'auto',
	                                   subsample= 0.9, min_samples_split= 2, min_samples_leaf= 2, learning_rate= 0.14)        ]
    ...
    def _apply_pca(data, labels, n_comps=1):
        ...
        """
        # PCA
        pca = PCA(n_components=n_comps, whiten=False, copy=True) # Orig
        # pca = PCA(n_components=0.99 , whiten=False, copy=True) #New / "mle"
    ...
    def fit(self, X, Y):
        ...
        'Maybe remove noise? '
        # self._noise = [random.uniform(-0.000005, 0.000005) for p in range(0,X.shape[1])] #ORIG
        #MODDED
        self._noise = 0
        ...
        for i in range(0,self._n_classifiers):
            ...
            # K = int(round(1 + NF/4*random.random())) #ORIG

            # 'changed'
            #NEW - up to K/3 features:
            K = int(round(2 + NF / 3 * random.random())) # NEW!
            ...
            for l in range(0,K):
                ...
                Xzij = Xz[:, vpos] #ORIG
# http://stackoverflow.com/questions/21506758/unhashable-type-error-with-sklearn-and-importing-a-csv
                # Xzij = Xz.iloc[vpos] # NEWNEW pandas
            ...
            # cl = self._listclassifiers[ensembleComposition[i]]() #ORIG
            #NEW:
            cl = self._listclassifiers[ensembleComposition[i]] #NEW

            # print("y_trainADAHERF") #NEW
